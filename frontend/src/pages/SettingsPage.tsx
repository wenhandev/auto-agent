...
export function SettingsPage() {
  ...
  const [testResult, setTestResult] = useState<LlmConfigTestResult | null>(
    null,
  );
  ...
  useEffect(() => {
    if (active && !hydrated) {
      ...
      const t = active.self_healing_vision_threshold;
      setSelfHealThreshold(typeof t === "number" ? t : 0.6);
      setHydrated(true);
    }
  }, [active, hydrated]);
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border bg-muted/40 px-4 py-3 text-xs">
                ...
                <span>
                  ...
                  <code className="font-mono">
                    {effectiveQuery.data.source}
                  </code>
                </span>
                <span>
                  {t("pages.settings.effectiveProvider")}:{" "}
                  <code className="font-mono">
                    {effectiveQuery.data.provider}
                  </code>
                </span>
                <span>
                  {t("pages.settings.effectiveModel")}:{" "}
                  <code className="font-mono">
                    {effectiveQuery.data.model}
                  </code>
                </span>
                <span>
                  {t("pages.settings.effectiveKey")}:{" "}
                  <code className="font-mono">
                    {effectiveQuery.data.api_key_masked}
                  </code>
                </span>
              </div>
            )}
            ...
            <div className="grid max-w-2xl gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder={
                    active
                      ? t("pages.settings.apiKeyKeep", {
                          masked: active.api_key_masked,
                        })
              ...
              <div className="grid gap-2">
                <Label htmlFor="settings-base">
                  {t("pages.settings.baseUrlLabel")}
                </Label>
                <Input
                  id="settings-base"
                  value={baseUrl}
